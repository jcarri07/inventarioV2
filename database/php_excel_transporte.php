...
header('Content-Disposition: attachment; filename=Exportación_Transporte.xlsx');
?>
    ...
    <table id="dataTables1" class="table table-bordered table-striped table-hover">
      ...
      <tbody>
        ...
        if ($permiso = !'Super Admin') {
          $query2 = mysqli_query($mysqli, "SELECT * FROM transporte WHERE sede LIKE '$sede' ORDER BY codigo DESC")
            or die('error: ' . mysqli_error($mysqli));
        } else {
          $query2 = mysqli_query($mysqli, "SELECT * FROM transporte ORDER BY codigo DESC")
            or die('error: ' . mysqli_error($mysqli));
        }
