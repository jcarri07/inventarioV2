<?php
  ...
  header('Content-Disposition: attachment; filename=Exportación_Comunicaciones.xlsx');
?>
    ...
    <table id="dataTables1" class="table table-bordered table-striped table-hover">
      ...
      <?php
          ...
          if($permiso != 'Super Admin') {
            $query2 = mysqli_query($mysqli, "SELECT * FROM inventario WHERE categoria = 'Comunicaciones' and sede LIKE '$sede' ORDER BY codigo DESC")
            or die('error: ' . mysqli_error($mysqli));
          } else {
            $query2 = mysqli_query($mysqli, "SELECT * FROM inventario WHERE categoria = 'Comunicaciones' ORDER BY codigo DESC")
            or die('error: ' . mysqli_error($mysqli));
          }
